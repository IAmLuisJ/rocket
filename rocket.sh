#!/bin/bash
# Rocket - Long-running AI agent loop
# Usage: ./rocket.sh [--help] [--once] [--max-iterations N] [N]
...
if [ ! -f "$PROGRESS_FILE" ]; then
  echo "# Rocket Progress Log" > "$PROGRESS_FILE"
...
show_rocket
echo -e " ${C}Starting Rocket${R} ・ ${Y}v$VERSION${R} ・ Max iterations: ${Y}$MAX_ITERATIONS${R}"
echo ""
...
for i in $(seq 1 $MAX_ITERATIONS); do
  ...
  # Run Agent with the Rocket prompt (prepend PROJECT_ROOT)
...
$(cat $SCRIPT_DIR/.agent/PROMPT.md)"
  ...
  if has_complete_tag "$OUTPUT" || has_complete_tag "$FINAL_SUMMARY"; then
    ...
    echo ""
    echo -e "${GR}░░▒▒▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▒▒░░${R}"
    echo -e "  🎉 ${GR}Rocket completed all tasks!${R}"
  ...
  if has_blocked_tag "$OUTPUT" || has_blocked_tag "$FINAL_SUMMARY"; then
    ...
    show_notification "Rocket - BLOCKED" "$BLOCKED_REASON"
  ...
  if has_decide_tag "$OUTPUT" || has_decide_tag "$FINAL_SUMMARY"; then
    ...
    ELAPSED_STR=$(format_duration $ELAPSED)
    play_notification_sound
    show_notification "Rocket - Decision Needed" "$DECIDE_QUESTION"
...
echo ""
echo -e "${Y}░░▒▒▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▒▒░░${R}"
echo -e "  ⚠️  ${Y}Rocket reached max iterations${R} (${M}$MAX_ITERATIONS${R})"
if [ ${#ITERATION_TIMES[@]} -gt 0 ]; then
